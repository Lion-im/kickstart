...
<div class="col-6 mx-auto text-center">
	...
	<?php 
		...
		$query = mysqli_query($connect, "SELECT * FROM projects WHERE  user='erik'");
	...
	<div class="col-4 p-3 text-dark mt-2 mx-auto" >
		<div class="col-12 bg-light rounded p-3" >
			...
			<div>
				<h3><!--Заголовок проекта--> <?php echo $post["title"]; ?></h3>
				<p><!--Описание проекта--> <?php echo $post["text"]; ?></p>
				<p><!--юзер--> <?php echo $post["user"]; ?></p>
				...
				<form action="deleteProject.php" method="GET">
					...
					<button class="btn btn-danger" href="deleteProject.php">Удалить проект</button>
				</form>
				<form action="updateProjectPick.php" method="GET">
					<input value="<?php echo $post["id"]; ?>" type="hidden" name="id">
					<button class="btn btn-success" href="updateProjectPick.php">Редактировать проект</button>
				</form>
			</div>
		</div>
	</div>	
